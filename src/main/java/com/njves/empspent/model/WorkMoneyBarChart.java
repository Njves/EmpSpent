...
public class WorkMoneyBarChart implements DataBarChart {
    ...
    public void fillSeries(WorkDay workDay, XYChart.Series<String, Double> series, XYChart.Series<String, Double> series1) {
        if(workDay.isWorked())
            series.getData().add(new XYChart.Data<>(workDay.getEmployee().getName(), workDay.getEmployee().getSpeciality().getSalary()));
        else
            series1.getData().add(new XYChart.Data<>(workDay.getEmployee().getName(), workDay.getEmployee().getSpeciality().getSalary()));
    }
